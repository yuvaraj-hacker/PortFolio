...
function Commoncard({ imagebright,bgStyle, first, second, websitecate, tech1, bgImage,imageStyle, tech2, tech3, techList = [], from = '#ffff', to = '#0000', link = '/folsum-pulse' }) {
    return (
        <> 
        {/* home card props except 1st three */}
            <section className='px-5'>
                <section className={`max-w-[80rem] mx-auto overflow-hidden bg-center bg-cover border-2 rounded-t-[3.5rem] rounded-bl-[3.5rem] w-full ${bgStyle} `} style={{ backgroundImage: `url(${bgImage})` }}>
                    <div className='grid md:grid-cols-5  md:gap-5'>
                        <div className='md:col-span-3 md:p-0 p-2'>
                            <img className={`${imageStyle} xl:h-[450px] p-5 `} src={imagebright} alt="" />
                        </div>
